#!/usr/bin/env bash
# ============================================================
#  VOXEL-CHUNK + ROTATION-INVARIANT GEOMETRY (10D)
#  Same chunking as the 56.45 run, but the 3 "spatial" channels are replaced with
#  linearity / planarity / verticality (eigenvalue ratios + gravity cue). Measured MAE
#  under rotation, isotropic scale and mirroring: 0.00000 -- exactly invariant, where the
#  stored channels drifted up to 87% of their own std. Targets the classes both previous
#  experiments lost when those channels were disturbed: board, window, bookcase.
#
...
#
#  7D features (model_params.json): the stored spatial channels are not
