...
PointEdgeSegNet is the lightweight 3D point cloud segmentation model based on EdgeConv, Transforer and U-Net architecture. This project has purpose of researching and developing 3D point cloud segmenation model in [3D Scan to BIM pipeline](https://github.com/mac999/scan_to_bim_pipeline) development that can classify each point in large-scale 3D PCD into semantic categories such as ceiling, floor, wall, furniture, and other objects which you can customize easily under small size VRAM like 24GB under. This is a personal project that I develop in my spare time, but if you're interested, you're welcome to join me in developing it at any time.
...
The model combines the effectiveness of EdgeConv for capturing local geometric relationships with the encoder-decoder structure of U-Net with Transformer for preserving spatial information across different scales. This approach enables accurate semantic segmentation of complex indoor scenes. 
