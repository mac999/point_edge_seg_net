...
While there are many research and open-source models for segmenting PCD, I found them challenging to use from a practical perspective. For example, when processing large PCD files, open models either didn't perform as well as the papers claimed or required additional development to handle them. Some model codes were so outdated that they were difficult to install and run in current development environments. Some models used specialized libraries or file formats for training, limiting their general usability. This project tries to eliminate these issues as much as possible.
...
Loss and Acc performance log(https://github.com/mac999/point_edge_seg_net/tree/main/logs/20251007_095926). Train and Val dataset = S3DIS v1.2 aligned (Area 1, 2, 3, 4, 6) 
</p>
