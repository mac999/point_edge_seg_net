...
| Q2 | TTA 채점 | ✅ 스케일-TTA는 chunk 모드에 미배선(no-op)이었음을 발견 → **D4 TTA**(격자 보존: rot90×flip 8뷰)를 신규 구현. E5 final: 64.23 → **64.58 (+0.35)**. 스케일 TTA는 stencil 격자를 깨므로 D4가 옳은 계열 |
| Q3 E6 | room 모드 × 300ep | ❌ **60.04** (best 59.49) — E5(64.23) 대비 −4.2. room-val 82.8로 높았지만 test 붕괴. v1 시절(StageE)에 이어 2번째 확인: **방 전체 문맥의 이득 < 학습 샘플 다양성 손실**(방 170개 vs 청크 1293개). chunk+halo가 최종 승자로 확정 |
| Q4 E7 | 2cm 복셀 × ep132 조기종료 | ❌ **60.37** (best 58.36) — E5(64.23) 대비 −3.9. 단 **설계 결함 발견**: core 24576@2cm의 바닥 면적은 12288@4cm의 **절반**(밀도 4배 vs 예산 2배) → 해상도 상승과 물리적 문맥 축소가 뒤섞임. 클래스 증거: window +3.5(해상도 이득) vs door −25.7·sofa −12.6(문맥 손실) — 공정한 2cm 검증은 block ~98k 필요(비용 대비 후순위) |

**v0.8.x 큐 최종 요약 (2026-08-17 23:45 완료)**: E5 ✅ 64.23 (+장기학습 입증) /
D4-TTA ✅ 64.58 / E6 ❌ 60.04 (room 모드 2차 기각 — chunk+halo 확정) /
E7 ❌ 60.37 (2cm, 문맥-해상도 혼입 결함). **공식 기록: 64.58** (E5 final + D4 8뷰).
이틀 누적 58.82→64.58 (+5.76). 다음 후보: beam 구제, DALES 이식, 방향별 가중치
stencil(spconv식), c1 확대(미검증 잔여 가설), 공정한 2cm(block 98k).

## E8 결과 (2026-08-18 19:01, run logs/20260818_003229)

원통형 이방성 stencil(stage1+, zero-init 게이팅) + beam weight 3.0, 600ep(ep~514 조기종료).
val best 85.02@ep444 (사상 최고) → test **final 64.25** (E5 64.23과 동률),
**final+D4TTA 64.79 = 신기록 (+0.21)**. best 63.49 < final (5번째 확인).

판정: **방향 게이팅의 총점 기여는 ~0** — 그러나 클래스 이동은 실재:
board +3.4(66.1), door +2.3, chair +1.8 vs column −4.6, window −3.0, sofa −1.8.
수평 구조(board=벽걸이 평판, 문 상단)는 얻고 수직 기둥·창은 오히려 잃음 — 원통
게이팅이 z-레벨 구분은 줬지만 수직 연속성(기둥이 여러 z를 관통) 모델링엔 부족.
beam: weight 3.0으로도 0.0 유지 — Area_5 GT 0.029%는 손실 가중으로 구제 불가 확정.
expandable_segments 효과 확정: VRAM 톱니 14→93GB → **11~20GB 안정** (전 구간).
...
## E10 결과 (2026-08-21) + E9/E10 라운드 종합

E10 (stencil z-reach 4, run logs/20260819_224432, ep~527 조기종료, val best 84.4):
test **final 62.92 / best 62.78 / +TTA 63.36 — E5 대비 −1.3, 기각**.
표적이던 column도 −1.4로 미회복 (window +1.6, door +2.3은 소득이나 sofa −9.8이 상쇄).
수직 도달 확장(K=225)은 이웃 수만 늘리고 신호를 희석 — column의 병목은 수용영역이
아니라 다른 곳(문맥/표현)에 있음이 2연속 확인(E8 게이팅, E10 도달 모두 실패).

**라운드 결론**: E9(−2.1)·E10(−1.3) 모두 기각 → **E5 구성(stencil r2+diff, 등방,
c1=64)이 국소 최적으로 확정**. 공식 기록 64.79 유지 (E5 final + D4 TTA).
아키텍처 미세조정 카드는 소진 — 남은 방향은 도메인 확장(DALES), 대안 패러다임
(병목→Mamba, 어텐션 확대), 앙상블/후처리 계열.

운영: 8/20 저녁부터 전력 캡 250W (온도 86→85°C, 팬 71→60%, 속도 −8%).
