...
echo "=== CHUNK START $(date '+%F %T') ==="
before=$(newest_run)
if bash run_train_chunk.sh; then
    run=$(newest_run)
    if [ -n "$run" ] && [ "$run" != "$before" ] && [ -f "${run}best_model.pth" ]; then
        echo "=== [CHUNK] TRAIN DONE -> $run $(date '+%F %T') ==="
        if "$PYTHON" evaluate_full.py --model_weights "${run}best_model.pth" \
            ...
            echo "=== [CHUNK] RESULT full-protocol mIoU=${miou} (best so far 54.47) run=${run} $(date '+%F %T') ==="
        else
            echo "=== [CHUNK] EVAL FAILED $(date '+%F %T') ==="
        fi
    else
        echo "=== [CHUNK] TRAIN FAILED (no new run) $(date '+%F %T') ==="
    fi
else
    echo "=== [CHUNK] TRAIN FAILED $(date '+%F %T') ==="
fi
echo "=== CHUNK COMPLETE $(date '+%F %T') ==="
